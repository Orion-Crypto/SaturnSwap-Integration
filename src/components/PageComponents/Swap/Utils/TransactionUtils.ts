...
export const Order = async (pool: any, swapType: any, orderType: any, setSwap: any, tokenBalance: any) => {
    if (swapType === SwapType.Market) {
        await MarketOrder(pool, orderType, tokenBalance);
...
export const LimitOrder = async (pool: any, orderType: any) => {
    let tokenAmountSell = convertStringToNumber((document.getElementById(`token_amount_sell`) as HTMLInputElement)?.value);
    let tokenAmountBuy = convertStringToNumber((document.getElementById(`token_amount_buy`) as HTMLInputElement)?.value);
...
export const MarketOrder = async (pool: any, orderType: any, tokenBalance: any) => {
    let tokenAmountSell = convertStringToNumber((document.getElementById(`token_amount_sell`) as HTMLInputElement)?.value);
    let tokenAmountBuy = convertStringToNumber((document.getElementById(`token_amount_buy`) as HTMLInputElement)?.value);
    ...
    let slippage: any = getSlippageValue();
    if (slippage === 'Auto' || slippage === 'auto' || slippage === 'Unlimited') {
        slippage = null;
    } else {
